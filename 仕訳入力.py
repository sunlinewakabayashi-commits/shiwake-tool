...
ACCOUNTING_FILE = "あかしまい_損益確認 R8年度分.xlsx"
SHEET_SUFFIX = "現金"
KAKERATE = 1.01
YEAR_PREFIX = "26"
...
PROMPT_TEXT = """この画像/ファイルに含まれるレシート・領収書・請求書をすべて読み取ってください。

レシートが1枚でも複数枚でも、必ず以下のJSON配列形式で返してください。
他のテキストは不要です。

【日付の読み取りについて重要な注意事項】
- 日付は必ずレシートに印刷・記載された数字をそのまま1文字ずつ正確に読み取ること
- 手書きの日付は特に慎重に読み取ること。「4」と「9」、「24」と「25」など似た数字の誤読に注意
- 「08日」を「09日」、「18日」を「19日」など末尾の数字を1つずれて読む誤読に特に注意
- 推測や補完で日付を変えないこと
- apollostationなどガソリンスタンドのレシートには「処理日付」が翌日になっている場合があるが、
  必ず上部の発行日時（例：「2026年04月25日 18:48」）を日付として使うこと。「処理日付」は無視すること

【時刻の読み取り】
- レシートに時刻が記載されている場合は必ず読み取り「時刻」フィールドに記入すること（例：「13:27」）
- 時刻がない場合は空文字とすること
- 同じ日付・同じ店舗でも時刻が異なれば別の取引として登録すること

【数字の読み取りについて重要な注意事項】
- 合計金額は「合計」「お買上合計」「ご請求額」「今回請求額」などの最終税込金額を読み取ること
...
- SuicaチャージなどICカードチャージの領収書（JR東日本・駅発行）も交通費として10%で計算すること
  店名はレシートに記載の発行駅・会社名（例：「JR東日本」「国分寺駅」）を使用すること
  消費税 = int(チャージ金額 × 10 / 110) で計算すること
  チャージ金額は「金１０００円」「金２０００円」「金５０００円」「金１００００円」が一般的。
  ゼロの個数を必ず数えて正確に読み取ること
- 「標準税率対象 ¥X」「軽減税率対象 ¥Y」のように税率区分の金額だけ記載され消費税額がない場合は以下で計算すること
  10%のみの場合: 消費税 = int(合計金額 × 10 / 110)
  8%のみの場合:  消費税 = int(合計金額 × 8 / 108)

【注文番号・伝票番号の読み取り】
- Amazon Pay / クリックポストなどの注文明細では「注文番号」「ご注文番号」「伝票番号」などを読み取り、
  「注文番号」フィールドに記入すること（例：「P03-3940711-5901247」）
- ゆうちょ・銀行の払込明細では「伝票番号」を注文番号フィールドに記入すること
- レシートに注文番号・伝票番号の記載がない場合は空文字とすること

【カテゴリ判定ルール】
以下のルールを優先順位順に適用すること：

1. 振込手数料・振替手数料 → 「雑費」
2. 振込手数料が本体と別に記載されている場合は別エントリで登録し「雑費」
3. ゆうちょ銀行・銀行の払込取扱票に振込料金が記載されている場合も別エントリで「雑費」
4. 道路使用許可・道路占用許可など警察署への支払い → 「賃借料」
5. 法人会・商工会・町内会・自治会・業界団体などへの会費・年会費 → 「接待交際費」
6. 廃棄物収集・ゴミ回収・清掃業者（藤ビルメンテナンス等）への支払い → 「衛生費」
7. 宅配便・運賃・運送料・送料・配送料・ヤマト運輸・佐川急便・日本郵便 → 「通信費」
8. 郵便局・ゆうちょ・郵便・宅配便・クリックポスト → 「通信費」
9. ドラッグストア（ツルハドラッグ・マツキヨ等）は購入品の税率で判定すること
   8%対象のみ → 「食材費」
   10%対象のみ → 「消耗品費」
   混在 → 税率内訳で分割
10. 特別区民税・都民税・住民税・固定資産税・森林環境税など税金の納付領収書 → 「預り金」
11. レストラン・食堂・定食屋・居酒屋・カフェ・ファストフード・弁当屋など外食・飲食店 → 「会議費」
12. クリーニング店・洗濯・衛生用品の購入 → 「衛生費」
13. 合計金額が1,000円未満でどのカテゴリにも当てはまらない場合 → 「雑費」
14. 上記以外 → 食材費/交通費/消耗品費/会議費/水道光熱費/通信費/衛生費/広告宣伝費/預り金/雑費/賃借料/接待交際費/その他 から最も適切なもの

【請求書の分割登録】
- 藤ビルメンテナンスの請求書は店舗別に分割して登録すること
  各明細行の金額は税抜のため ×1.1 で税込を計算すること
  店舗名は請求書内の顧客コード行（例：<<忍者喫茶不忍秋葉原萬世城 様>>）から読み取ること
  各店舗のエントリの「店名」は「藤ビルメンテナンス」、「メモ」に店舗名を記入すること
- ゆうちょ銀行・銀行の払込票で振込料金が別途記載されている場合は、
  本体と振込料金を別エントリで登録すること
...
よく見られる表記と読み方：
- ファミリーマート（レジ発行レシート）: 全角数字で税率が書かれる
  １０％対象日用品等 ¥Y → 10%の税込金額
  （内消費税等 ¥B） → 10%の消費税
  ８％対象持帰食品等 ¥X → 8%の税込金額
  （内消費税等 ¥A） → 8%の消費税
  ※読み取り後に必ず検算すること: X + Y = 合計金額
- ファミリーマート（領収証形式）:
  「１０％対象 ¥Y（内消費税等 ¥B）」「８％対象 ¥X（内消費税等 ¥A）」
  Xが税込金額、Aが消費税額。税抜き = X - A で計算すること
...
- イオン・ピーコックストア（外税形式）:
  ...
  税率内訳["8%"]["税込金額"] = X + A
  税率内訳["10%"]["税込金額"] = Y + B
- カクヤス: レシート右側縦向きに税率内訳が印刷される
  「消費税(8%) ¥A」「8%対象額 ¥X」「消費税(10%) ¥B」「10%対象額 ¥Y」を正確に読み取ること
  8%は食材費（酒類）、10%は少額なら雑費として分割登録すること
  税込合計と税率内訳の合計が一致するか必ず検算すること

[
  {
    "日付": "YYYY/MM/DD形式（不明な場合は今日の日付）",
    "時刻": "HH:MM形式（なければ空文字）",
    "店名": "店舗名（株式会社等の法人格は除く。レシートの文字をそのまま正確に読み取ること）",
    "合計金額": 数値のみ（税込合計）,
    "消費税": 数値のみ（合計消費税、不明な場合は0）,
    "カテゴリ": "上記カテゴリ判定ルールに従った勘定科目",
    "メモ": "特記事項があれば（なければ空文字）",
    "注文番号": "注文番号・伝票番号（なければ空文字）",
    "税率内訳": {
      "8%":  {"税込金額": 数値, "消費税": 数値},
      "10%": {"税込金額": 数値, "消費税": 数値}
    }
...
def read_receipts_from_file(filepath):
    ...
    if ext == ".pdf":
        file_block = {"type": "document", "source": {"type": "base64", "media_type": "application/pdf", "data": file_data}}
    else:
        media_types = {".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".png": "image/png", ".webp": "image/webp", ".gif": "image/gif"}
        file_block = {"type": "image", "source": {"type": "base64", "media_type": media_types.get(ext, "image/jpeg"), "data": file_data}}
        ...
        try:
            ...
            while pos != -1 and pos < len(text):
                try:
                    ...
                    result.append(obj); pos = text.find("{", pos_next)
    ...
    round_num = 1
    while True:
        response = client.messages.create(model="claude-opus-4-6", max_tokens=8192, messages=messages)
        ...
        messages.append({"role": "user", "content": "まだ読み取っていないレシートがあれば、同じJSON配列形式で出力してください。すべて読み取り済みであれば [] とだけ返してください。"})
...
def get_last_state_readonly(accounting_path, sheet_name):
    try:
        ...
        if sheet_name not in wb.sheetnames:
            wb.close(); return 0, None
        ws = wb[sheet_name]
        last_balance = 0; last_number = None
...
def create_output_excel(rows, output_path):
    ...
    widths = [12, 12, 14, 14, 14, 20, 12, 12, 10, 14, 10, 12, 35]
...
def main():
    ...
    for filepath in filepaths:
        ...
        try:
            ...
            log(f"エラー: {filepath} の読み取りに失敗しました: {e}"); continue
        ...
        # -------------------------------------------------------
        # 重複除外: キーに注文番号・時刻を追加（修正点①②）
        # → 同日・同店・同額でも注文番号or時刻が違えば別件として登録
        # -------------------------------------------------------
        seen    = set()
        deduped = []
        for r in receipts:
            order_number = r.get("注文番号", "") or ""
            jikoku       = r.get("時刻", "") or ""
            key = (r.get("日付", ""), r.get("店名", ""), int(r.get("合計金額", 0)), order_number, jikoku)
            if key in seen:
        ...
        for i, r in enumerate(receipts, 1):
            order_num = r.get("注文番号", "") or ""
            jikoku    = r.get("時刻", "") or ""
            suffix    = f" [{order_num}]" if order_num else (f" {jikoku}" if jikoku else "")
            log(f"  │ {i:2}. {r.get('日付','')} {r.get('店名','')[:16]:<16} {int(r.get('合計金額',0)):>8,}円{suffix}")
        log(f"  └" + "─" * 38)
        log(f"")
        ...
        for receipt in receipts:
            # 税率分割行は同一管理番号（日付・店名・注文番号・時刻が同じ）
            is_split = (
                prev_receipt is not None
                and receipt.get("日付") == prev_receipt.get("日付")
                and receipt.get("店名") == prev_receipt.get("店名")
                and (receipt.get("注文番号", "") or "") == (prev_receipt.get("注文番号", "") or "")
                and (receipt.get("時刻", "") or "") == (prev_receipt.get("時刻", "") or "")
            )
            if is_split:
                kanri = current_kanri
            else:
            ...
            # 備考：注文番号＋時刻＋メモ
            memo         = receipt.get("メモ", "") or ""
            order_number = receipt.get("注文番号", "") or ""
            jikoku       = receipt.get("時刻", "") or ""
            parts = [p for p in [order_number, jikoku, memo] if p]
            memo_final = " ".join(parts)

            row = [
                kanri, None, company,
                date_str_to_excel(receipt.get("日付", "")),
                receipt.get("カテゴリ", ""),
                receipt.get("店名", ""),
                total, subtotal, None, new_balance, KAKERATE, billing,
                memo_final
            ]
            all_rows.append(row)
